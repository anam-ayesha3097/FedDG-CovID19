...
def epoch_site_train_moon(epochs, site_name, local_model, previous_local_model, global_model, optimzier, scheduler, dataloader, log_ten, metric):
    temperature = 1
    mu = 0.1
...
def site_train_moon(comm_rounds, site_name, args, local_model, previous_local_model, global_model, optimizer, scheduler, dataloader, log_ten, metric):
    tbar = tqdm(range(args.local_epochs))
    for local_epoch in tbar:
        tbar.set_description(f'{site_name}_train')
        epoch_site_train_moon(comm_rounds*args.local_epochs + local_epoch, site_name, local_model, previous_local_model, global_model, optimizer, scheduler, dataloader, log_ten, metric)
