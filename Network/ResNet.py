...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, backbone=True, feature_norm=False):
...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.l1(x)
        x = F.relu(x)
        projected_features = self.l2(x)
        # Classifier
        output = self.l3(projected_features)
